...
public class QxFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener {
    ...
    private List<ResultBean.DataBean> teamList = new ArrayList<>();
    ...
    private void initView() {
        ...
        adapter = new RecyclerCommonAdapter<ResultBean.DataBean>(mContext, teamList, R.layout.item_home) {
            ...
            public void convert(RecyclerViewHolder holder, final ResultBean.DataBean item, int position) {
                ...
                holder.setText(R.id.text_name,"七星彩");
                holder.setText(R.id.tv_time,"第" + item.getExpect() + "期开奖结果");
                holder.setText(R.id.tv_date,"开奖时间："+item.getOpentime());
                holder.setImageResource(R.id.img_flag,R.drawable.lottery_qixing);

                String data = teamList.get(position).getOpencode();
                ...
                codeAdapter = new RecyclerCommonAdapter<Integer>(mContext, codeList, R.layout.item_qixin) {
                    @Override
                    public void convert(RecyclerViewHolder holder, Integer item, int position) {
                        holder.setText(R.id.tv_code,String.valueOf(item));
                        holder.getView(R.id.tv_code).setBackground(getResources().getDrawable(R.drawable.shape_qixin));
                    }
                };
                ...
                holder.setOnClickListener(R.id.rl_rootview, new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        mContext.startActivity(new Intent(mContext,FcActivity.class)
                                .putExtra("title","7星彩")
    ...
    private void initData() {
        new OkHttpUtil().get(Constants.BASE_URL + "/qxc-20.json", new OkHttpUtil.HttpCallback() {
            @Override
            public void onSuccess(String data) {
                ...
                if(bean != null && bean.getData().size() > 0){
                    teamList.addAll(bean.getData());
                    adapter.notifyDataSetChanged();
                    SpUtil.save("tab","find_list",bean);     //缓存bean对象
                }
            }
